 ...
 *
 * The UI is a 420 x 480 fixed-size X11 window drawn with Cairo. The visual
 * design echoes the real Xfer Records OTT plugin's dark skin: a near-black
 * panel, an amber/gold accent color (matching the OTT wordmark), and a
 * black three-band level display with green (downward) and amber (upward)
 * compression zones either side of a dark gap at the threshold. Layout
 * mirrors the real plugin's grouping: top row is DEPTH/TIME/IN/OUT, the
 * band display sits below that, then per-band THRESHOLD/GAIN knobs for
 * HIGH/MID/LOW, and a final row of UPWARD/DOWNWARD knobs with a bypass
 * button between them, just like the controls below Xfer's band display.
 * Each knob is drawn as a modern rotary: outer ring with a colored value
 * arc sweeping 270 degrees, dark inner circle, and a white pointer. The
 * window is branded "OTT" / "by Zero:Archive" with credits to the original.
 *
...
enum {
    ...
    OTT_PORT_BAND3_GAIN,
    OTT_PORT_BYPASS,
    OTT_PORT_CONTROL_COUNT,     /* one past the last draggable widget port */
    OTT_PORT_IN_L = OTT_PORT_CONTROL_COUNT,
    OTT_PORT_IN_R,
    OTT_PORT_OUT_L,
    OTT_PORT_OUT_R,
    OTT_PORT_BAND1_METER,       /* output, dB: net gain on the low band  */
    OTT_PORT_BAND2_METER,       /* output, dB: net gain on the mid band  */
    OTT_PORT_BAND3_METER,       /* output, dB: net gain on the high band */
    OTT_PORT_TOTAL_COUNT
};
...
typedef struct {
    ...
    /* Live per-band meter values from the plugin's output control ports,
     * in dB (positive = net attenuation/downward, negative = net boost/
     * upward). Index 0=low, 1=mid, 2=high, matching the DSP's band order.
     * Updated by port_event, read by on_expose to position the meter's
     * white level marker. */
    float band_meter_db[3];
...
/* OTT dark theme matching the real Xfer OTT: near-black panel, dark gray
 * knobs, and the amber/gold accent color used for the OTT wordmark and
 * value arcs. The band display uses green for the downward-compression
 * zone and amber for the upward-compression zone, split by a dark gap
 * around the threshold, mirroring the real plugin's meter. */
#define C_BG          0.106, 0.106, 0.114   /* #1B1B1D near-black panel   */
#define C_KNOB_OUTER  0.937, 0.937, 0.941   /* #EFEFF0 white knob body    */
#define C_KNOB_INNER  0.851, 0.851, 0.859   /* #D9D9DB light knob center  */
#define C_ARC_ACTIVE  0.949, 0.663, 0.231   /* #F2A93B amber active arc   */
#define C_ARC_TRACK   0.290, 0.290, 0.298   /* #4A4A4C dark track         */
#define C_POINTER     0.106, 0.106, 0.114   /* near-black pointer         */
#define C_LABEL       0.898, 0.898, 0.898   /* #E5E5E5 knob labels        */
#define C_VALUE       0.604, 0.604, 0.612   /* #9A9A9C value text         */
#define C_BAND        0.929, 0.929, 0.933   /* #EDEDEE band labels        */
#define C_SEP         0.200, 0.200, 0.208   /* #333335 separator lines    */
#define C_TITLE       0.949, 0.663, 0.231   /* #F2A93B amber title        */
#define C_SUBTITLE    0.541, 0.541, 0.549   /* #8A8A8C dim subtitle       */
#define C_CREDITS     0.431, 0.431, 0.439   /* #6E6E70 credits            */
#define C_BYPASS_ON   0.122, 0.682, 0.290   /* #1FAE4A green (ACTIVE)     */
#define C_BYPASS_OFF  0.522, 0.157, 0.157   /* #851212 dark red (BYPASS)  */
#define C_BUTTON_TEXT 1.000, 1.000, 1.000   /* white button text          */
#define C_METER_BG    0.039, 0.039, 0.043   /* #0A0A0B meter background   */
#define C_METER_GREEN 0.239, 0.745, 0.361   /* #3DBE5C downward zone      */
#define C_METER_AMBER 0.910, 0.765, 0.290   /* #E8C34A upward zone        */
#define C_METER_LEVEL 1.000, 1.000, 1.000   /* white level marker         */
...
init_widgets(ott_ui_t *ui)
{
    ...
    /* Final row: UPWARD and DOWNWARD knobs flanking the bypass button,
     * below the band knobs -- matching the real OTT, where these two
     * knobs sit below the band display rather than beside it. cy=410. */
    KNOB(SIZE_SMALL, OTT_PORT_UPWARD,   "UPWARD",    70.0, 410.0, 0.0f, 1.0f, 1.0f);
    KNOB(SIZE_SMALL, OTT_PORT_DOWNWARD, "DOWNWARD", 350.0, 410.0, 0.0f, 1.0f, 1.0f);

    /* Bypass button centered between the UP/DOWN knobs. */
    TOGGLE(OTT_PORT_BYPASS,  "BYPASS",   165.0, 399.0, 90.0, 22.0, 0.0f);
...
on_expose(ott_ui_t *ui, const PuglExposeEvent *event)
{
    ...
    /* Meter section: black band display, 3 horizontal meters, full width
     * now that UP/DOWN knobs have moved to their own row below. Each bar
     * shows a green zone (downward compression, loud content pulled down)
     * and an amber zone (upward compression, quiet content pulled up)
     * split by a dark gap around the threshold -- the same visual
     * language as the real OTT's band display. */
    set_rgb(cr, C_METER_BG);
    ...
    cairo_select_font_face(cr, "sans-serif",
                           ...
                           CAIRO_FONT_WEIGHT_BOLD);
    cairo_set_font_size(cr, 10.0);
    const char *m_labels[3] = {"H", "M", "L"};
    const double bar_x = 34.0;
    const double bar_w = 340.0;
    const double bar_h = 14.0;
    /* dB range the marker sweeps across the bar before clamping to the
     * edge; matched loosely to how hard OTT's defaults tend to work. */
    const double meter_scale_db = 18.0;
    for (int m = 0; m < 3; m++) {
        double my = 108.0 + 165.0 * (0.25 + 0.25 * m);
        /* Row order is H/M/L top-to-bottom; band_meter_db is low/mid/high. */
        int band_idx = 2 - m;

        /* Label (H/M/L) to the left of the bar. */
        set_rgb(cr, C_LABEL);
        cairo_move_to(cr, 16.0, my + 4.0);
        cairo_show_text(cr, m_labels[m]);

        /* Downward zone (green), left portion. */
        set_rgb(cr, C_METER_GREEN);
        cairo_rectangle(cr, bar_x, my - bar_h / 2.0, bar_w * 0.42, bar_h);
        cairo_fill(cr);

        /* Threshold gap (dark), center portion. */
        set_rgb(cr, C_METER_BG);
        cairo_rectangle(cr, bar_x + bar_w * 0.42, my - bar_h / 2.0,
                        bar_w * 0.16, bar_h);
        cairo_fill(cr);

        /* Upward zone (amber), right portion. */
        set_rgb(cr, C_METER_AMBER);
        cairo_rectangle(cr, bar_x + bar_w * 0.58, my - bar_h / 2.0,
                        bar_w * 0.42, bar_h);
        cairo_fill(cr);

        /* White level marker: driven by the plugin's live per-band meter.
         * Positive dB (net attenuation) slides it left into the green
         * zone; negative dB (net boost) slides it right into the amber
         * zone; 0 sits at the threshold gap in the center. */
        double meter_db = (double)ui->band_meter_db[band_idx];
        double offset = -meter_db / meter_scale_db;
        if (offset < -1.0) offset = -1.0;
        if (offset > 1.0)  offset = 1.0;
        double marker_x = bar_x + bar_w * 0.5 + offset * (bar_w * 0.5);
        set_rgb(cr, C_METER_LEVEL);
        cairo_set_line_width(cr, 2.0);
        cairo_move_to(cr, marker_x, my - bar_h / 2.0 - 3.0);
        cairo_line_to(cr, marker_x, my + bar_h / 2.0 + 3.0);
        cairo_stroke(cr);
        ...
        cairo_set_font_size(cr, 9.0);
        char valbuf[16];
        snprintf(valbuf, sizeof(valbuf), "%+.1f", meter_db);
        cairo_move_to(cr, bar_x + bar_w + 8.0, my + 3.0);
           ...
           const void  *buffer)
{
    ...
    /* Live band meters: not a draggable widget, just cached for on_expose. */
    if (port_index == OTT_PORT_BAND1_METER ||
        port_index == OTT_PORT_BAND2_METER ||
        port_index == OTT_PORT_BAND3_METER) {
        int b = (int)(port_index - OTT_PORT_BAND1_METER);
        if (v != ui->band_meter_db[b]) {
            ui->band_meter_db[b] = v;
            if (ui->view) puglObscureView(ui->view);
        }
        return;
    }
